<script>
  ...
  const HISTORY_VIS_KEY = 'add_history_show'

  function randInt(min, max) {
    return Math.floor(Math.random() * (max - min + 1)) + min
  }

  function pickNoCarryTwoDigitPair() {
  ...
  function pickCarryTwoDigitPair() {
    const onesA = randInt(0, 9)
    const onesB = randInt(Math.max(0, 10 - onesA), 9)
  ...
  function newProblem() {
    ...
    if (level === '3') {
      ;[a, b] = pickNoCarryTwoDigitPair()
      regenChoices()
      return
    }
    ;[a, b] = pickCarryTwoDigitPair()
    regenChoices()
  }
  ...
  $: levelTitle =
    level === '1'
      ...
      : level === '2'
        ? 'Level 2: single digit + two digit'
        : level === '3'
          ? 'Level 3: two digit + two digit (no carry)'
          : 'Level 4: two digit + two digit (with carry)'
